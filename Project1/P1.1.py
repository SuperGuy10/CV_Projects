    ...
    for j in range(0,inputTemp.shape[1]):
        ...
        if (inputTemp[i, j, 0] < mL):
            ...
            if (inputTemp[i,j,0] > ML):
                ...
                if (mL != ML):
                    inputTemp[i, j, 0] = (inputTemp[i, j, 0] - mL) * 100 / (ML-mL)
        if (inputTemp[i, j, 0] == 100):
            outputTemp[i, j] = [255, 255, 255]
        else:
            #in case the L is 0
            if (inputTemp[i, j, 0] == 0):
                outputTemp[i, j] = [0, 0, 0]
            else:
                if (b == g and b == r and r == g):
                    inputTemp[i, j, 1] = -0.729411764706
                    inputTemp[i, j, 2] = -0.266666666667
                inputTemp[i, j]=np.array(Luv2XYZ(inputTemp[i, j]), np.float_)
                inputTemp[i, j]=np.array(np.dot(XYZ2RGB, inputTemp[i, j].T), np.float_)
                inputTemp[i, j]=np.array(limit(inputTemp[i, j]), np.float_)
